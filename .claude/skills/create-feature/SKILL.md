...
- **Installed versions are the source of truth.** Read `package.json` and the
  ...
  Currently: Next 15.5.x, React 19.x, Zod 3.25.x, Tailwind v4, BlockNote 0.51.x.
- **Anthropic / Claude API work** → invoke the `claude-api` skill for current
  model IDs, params, and SDK usage. Do not hand-write model IDs from memory.
...
truth for any persisted data, APIs were verified against installed versions, and
`verify.mjs` passes clean. Summarize for the user what you added (by file), what
you verified against, and the gauntlet result.
